...
def run_agent(client: anthropic.Anthropic, prompt: str) -> str:
    ...
    # web_search_20260209's dynamic filtering runs server-side code execution in a
    # container. When the server tool loop pauses (pause_turn) mid-execution, the
    # continuation round must reuse that same container, or the API rejects it with
    # "container_id is required when there are pending tool uses generated by code
    # execution with tools". Carry the container id across rounds.
    container_id = None
    ...
    for _ in range(15):  # safety cap on continuation rounds
        ...
        # Carry the code-execution container across continuation rounds.
